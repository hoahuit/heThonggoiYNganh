...
// Kiểm tra nếu giáo viên đã đăng nhập, chuyển hướng về trang chính
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("SELECT * FROM GiaoVien WHERE TenDangNhap = ? AND MatKhau = ?");
    $stmt->execute([$tenDangNhap, $matKhau]);
    $giaoVien = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($giaoVien) {
        // Đăng nhập thành công, khởi tạo session
        $_SESSION['MaGiaoVien'] = $giaoVien['MaGiaoVien'];
        $_SESSION['HoTen'] = $giaoVien['HoTen'];
        header("Location: index.php");
        exit();
    } else {
